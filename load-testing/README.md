...
3. Run Locust (since everything is in config files, no command line args are needed):

   ```bash
   locust
   ```
4. View the results live in the log output, and in `/results` after the test run completes.