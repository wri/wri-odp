...
function TeamProfile({ user }: { user: IRowProfile | IUser }) {
    ...
    user.image_display_url = (user?.image_display_url as string)
        ...
        <div className="flex flex-col sm:flex-row py-3 pl-4 sm:pl-8 gap-x-14 gap-y-6">
            ...
            <div className="font-normal text-base sm:text-[14px] text-wri-black sm:self-center sm:ml-auto sm:mr-[20%] lg:mr-[46%]">
                {UserProfile?.orgnumber
                    ? `Member of ${UserProfile?.orgnumber} Teams`
                    : 'No Teams'}
            </div>
        </div>
    )
            ...
            {UserProfile?.orgs?.map((team, index) => {
                return (
                    <div key={team.name}>
                        <Row
                            authorized={
                                team.userCapacity?.toLowerCase() === 'admin'
                                    ? true
                                    : false
                            }
                            key={index}
            ...
            <SearchHeader
                ...
                Pagination={
                    <Pagination
                        ...
                        count={processedData.data?.count}
                    />
                }
            />
