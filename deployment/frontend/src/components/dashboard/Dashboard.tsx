    ...
    return (
        <div className="relative max-w-8xl mx-auto w-full py-12 px-4 sm:px-6 lg:px-8">
            ...
            <ReactSortable
                list={state}
                setList={setState}
                className={`w-[90%] mx-auto sm:w-full h-full grid grid-cols-1 sm:grid-cols-2 sm:grid-rows-2 gap-6 xxl:gap-6 ${drag.sortable ? 'grid' : 'hidden'}`}
                sort={true}
                style={{ zIndex: 9999 }}
            >
                {state.map((item) => (
            ...
            <div
                className={`w-[90%] mx-auto sm:w-full h-full grid grid-cols-1 sm:grid-cols-2 sm:grid-rows-2 gap-6 xxl:gap-6 ${drag.sortable ? 'hidden' : 'grid'}`}
            >
                {state.map((item) => (
                    <div
                        key={item.id}
                        className="  w-full sm:h-[463px] relative"
                    >
