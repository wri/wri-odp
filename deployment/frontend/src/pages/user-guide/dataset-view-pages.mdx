...
Starting from the <mark>Open in GFW</mark> button, the button enables us to open the same dataset layer in Global Forest Watch which is one of WRI’s products.
...
Sometimes a dataset can have global tabular data associated with it, such tabular can be previewed in other external platforms such as <mark>cartodb</mark> hence, if you click on <mark>Open table in Carto</mark> it opens the tabular data in Cartodb portal. Also if you click on <mark>View Table Preview</mark>instead of opening the tabular data in Cartodb, the Preview section will display a tabular preview of data fetched from Cartodb as shown below
