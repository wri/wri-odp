...
For the preview section, the image display above shows Map preview, and that's because the dataset contains a layer data file, and in cases where no layer data files is available, but tabular data filess are present, the preview section will contain a tabular preview and if no data filess at all, the preview section will be empty
...
The <mark>RDI approved</mark> tags show when a Technical note is added. The csv , XLSX , LAYER show that this dataset contains three types of data filess.
...
First, we have the search section, which enables us to search data filess based on title and description. To search by location, click on the <mark>Location Search</mark> to enable the map view and map search
...
contains all the default or filtered data filess

<img src="/images/user-guide/dataset10.webp" />

#### Data file download

As shown in the image above, we have different control buttons e.g Select all data filess this selects all the tabular data files and then displays the Download selected data filess to download all selected files as a zipped file
...
Also, we have the Show all layers and Hide All this to display and hide the combined map view of all layer data filess.
...
contains detailed information on how to use provided APIs across different programming language to create, edit, and view datasets and data filess
