...
export const UserRouter = createTRPCRouter({
    ...
    deleteUser: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            const response = await fetch(
                `${env.CKAN_URL}/api/3/action/user_delete`,
                {
                    method: 'POST',
    ...
    deleteMember: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            const response = await fetch(
                `${env.CKAN_URL}/api/3/action/organization_member_delete`,
                {
                    method: 'POST',
                    body: JSON.stringify({
                        id: input.orgId,
    ...
    deleteApiToken: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            const res = await fetch(
                `${env.CKAN_URL}/api/3/action/api_token_revoke`,
                {
                    method: 'POST',
                    body: JSON.stringify(input),
                    headers: {
    ...
    createApiToken: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            const res = await fetch(
                `${env.CKAN_URL}/api/3/action/api_token_create`,
                {
                    method: 'POST',
                    body: JSON.stringify(input),
                    headers: {
    ...
    createOtherUser: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            if (input.team?.value) {
                const response = await fetch(
                    `${env.CKAN_URL}/api/3/action/organization_member_create`,
                    {
                        method: 'POST',
                        body: JSON.stringify({
                            id: input.team?.id,
