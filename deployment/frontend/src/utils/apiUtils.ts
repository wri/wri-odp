...
=======
  try {
    ...
    if (isSysadmin) {
      let urLink = ''
      if (q) {
        urLink = `${env.CKAN_URL}/api/3/action/${group_type == 'group'
            ? 'group_list_wri'
            : 'organization_list_wri'
          }?include_extras=true&all_fields=true&q=${q}`
      } else {
  ...
  try {
    const orgList = await Promise.all(
      [0, 1, 2, 3, 4, 5].map(async (i) => {
        const response = await fetch(
          `${env.CKAN_URL
          }/api/3/action/organization_list?include_extras=true&all_fields=true&limit=${(i + 1) * 25
          }&offset=${i * 25}`,
          {
            headers: {
              Authorization: apiKey,
      ...
      if (recipientUsers) {
        await Promise.all(
          recipientUsers
            ...
            .map(async (user) => {
              const subject = `Issue ${action} on Dataset ${dataset.title}`
              const body = `<p>Hi ${user.name ?? user.display_name ?? 'There'
                }</p>
                        <p>There has been an issue ${action} on the Dataset ${dataset.title
                }.</p>`
              const email = user.email!
              return await sendEmail(email, subject, body)
            })
          ...
          recipientUsers
            ...
            .map(async (user) => {
              ...
              const body = `<p>Hi ${user.name ?? user.display_name ?? 'There'
                        ...
                        <p>The approval status for the Dataset <a href="${env.NEXTAUTH_URL
                }/datasets/${dataset.name}">${dataset.title
                }</a> is now <b><strong>${mainAction}</strong><b></p>`
              const email = user.email!
              return await sendEmail(email, subject, body)
            })
