...
describe("Can add and remove members from teams and topics", () => {
  ...
  it("Create API Token", () => {
    ...
    cy.get('#name', { timeout: 10000 }).should('be.visible').clear().type("test-token");
    cy.contains("Create Token", { timeout: 10000 }).click();
    cy.contains("Successfully created the test-token token", { timeout: 20000 }).should('be.visible');
    cy.get("#copyButton", { timeout: 20000 }).should('be.visible').click();
    cy.contains("Copied text to clipboard", { timeout: 20000 }).should('be.visible');
  });

  it("List api keys", () => {
    cy.visit(`/dashboard/settings/edit/${adminUser}`);
    cy.contains("API Tokens", { timeout: 20000 }).click();
    cy.contains("test-token", { timeout: 20000 }).should('be.visible');
  });
  ...
  it("Delete api keys", () => {
    ...
    cy.contains("API Tokens", { timeout: 20000 }).click();
    cy.contains("test-token", { timeout: 20000 }).should('be.visible');
    cy.get('.delete-token', { timeout: 20000 }).should('have.length.greaterThan', 0).first().click();
    cy.contains('Delete Token', { timeout: 10000 }).click();
    cy.contains('Successfully deleted the test-token token', { timeout: 20000 }).should('be.visible');
    cy.contains("test-token").should('not.exist');
  });
