...
def test_location_search_1(mail_user):
    ...
    result_1 = get_action("package_create")(context=context, data_dict=dataset)

    search_1 = get_action("package_search")(
        context=context, data_dict={"ext_location_q": "10,10", "ext_address_q": "test"}
    )["results"]
...
@mock.patch("ckan.plugins.toolkit.mail_user")
@pytest.mark.usefixtures("with_plugins", "test_request_context")
def test_location_search_2(mail_user):
    userobj = factories.Sysadmin()
    session = model.Session
    context = {
        "model": model,
    ...
    search_2 = get_action("package_search")(
        context=context, data_dict={"ext_location_q": "9,40", "ext_address_q": ""}
    )["results"]
...
@mock.patch("ckan.plugins.toolkit.mail_user")
@pytest.mark.usefixtures("with_plugins", "test_request_context")
def test_location_search_3(mail_user):
    userobj = factories.Sysadmin()
    session = model.Session
    context = {
        "model": model,
    ...
    dataset["spatial_address"] = "Sao Paulo, Sao Paulo, Brazil"
...
@mock.patch("ckan.plugins.toolkit.mail_user")
@pytest.mark.usefixtures("with_plugins", "test_request_context")
def test_location_search_4(mail_user):
    userobj = factories.Sysadmin()
    session = model.Session
    context = {
        "model": model,
    ...
    dataset["name"] = "dataset-z-address"
    try:
        get_action("dataset_purge")(
            context={"ignore_auth": True}, data_dict={"id": dataset["name"]}
        )
    ...
    search_2 = get_action("package_search")(
        context=context, data_dict={"ext_location_q": "-8,-35", "ext_address_q": ""}
    )["results"]
