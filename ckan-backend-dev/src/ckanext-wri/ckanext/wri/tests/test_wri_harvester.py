...
class TestWriCkanHarvester(object):
    ...
    def test_default_extras(self):
        ...
        assert extras_dict["harvest_url"] == "http://localhost:8998/dataset/b1d735e9-cbe1-42f7-ad84-1cb1cb6fd327"
    ...
    def test_create_resources_flag_false(self):
        ...
        source_resources = mock_ckan.DATASETS[3]["resources"]
        source_resource_names = [
            r["name"] for r in source_resources if "http://localhost:8998/files/" in r["url"]
        ]

        for r in resources:
            if r["name"] in source_resource_names:
                assert ckan_site_url not in r["url"]
