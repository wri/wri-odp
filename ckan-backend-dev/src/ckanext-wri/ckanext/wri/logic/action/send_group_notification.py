        ...
        if recipient_users:
            for recipient_user in recipient_users:
                if recipient_user.get("email"):
                    ...
                    body = f"<p>Hi {recipient_user['display_name'] if recipient_user.get('display_name') else recipient_user['name']}</p><p>The approval status for the dataset <a href='{config.get('ckanext.wri.frontend_url')}datasets/{dataset['name']}'>${dataset['title']}</a> is now <b><strong>${mainAction}</strong><b></p>"
                    tk.mail_user(user_obj, subject, None, body)
