...
def pending_dataset_show(context: Context, data_dict: DataDict):
    ...
    try:
        ...
        include_rsc_spatial_data = data_dict.get('include_rsc_spatial_data')

        if include_rsc_spatial_data and pending_dataset is not None:
            resources = pending_dataset.get("resources", [])
            for i, resource in enumerate(resources):
                resource_id = resource.get("id")
                resource_location = ResourceLocation.get(resource_id=resource_id, is_pending=True)
                if resource_location is not None:
                    spatial_geom = resource_location.get("spatial_geom")
                    spatial_address = resource_location.get("spatial_address")
                    if spatial_geom is not None:
                        resources[i]["spatial_geom"] = json.loads(spatial_geom)
                    resources[i]["spatial_addess"] = spatial_address
...
def package_show(original_action, context, data_dict):
    ...
    if include_rsc_spatial_data:
        for i, resource in enumerate(resources):
            resource_id = resource.get("id")
            resource_location = ResourceLocation.get(resource_id=resource_id, is_pending=False)
            if resource_location is not None:
                spatial_geom = resource_location.get("spatial_geom")
                spatial_address = resource_location.get("spatial_address")
                if spatial_geom is not None:
