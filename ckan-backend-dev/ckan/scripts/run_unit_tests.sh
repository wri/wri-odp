...
if [ -d "src_extensions/ckanext-wri" ]; then
  ...
  pytest --ckan-ini=test.ini -v --tb=short ckanext/wri/tests 2>&1 | tee -a "$ROOT_DIR/test_results.txt"
...
for dir in ckanext-*; do
  if [ -d "$dir" ]; then
    ...
    pytest --ckan-ini=test.ini -v --tb=short ckanext/${dir#ckanext-}/tests 2>&1 | tee -a "$ROOT_DIR/test_results.txt"
    PYTEST_EXIT_CODE=${PIPESTATUS[0]}
